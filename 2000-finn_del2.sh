...
def get_db_connection():
    try:
        ...
        return None
EOF
if ! sudo mv "$TEMP_UTILS_PY" /var/www/finn/2100-utils.py >> "$LOGFILE" 2>&1; then
    echo "❌ Feil: Kunne ikke opprette 2100-utils.py." | tee -a "$LOGFILE"
    exit 1
fi
sudo chown www-data:www-data /var/www/finn/2100-utils.py >> "$LOGFILE" 2>&1
sudo chmod 644 /var/www/finn/2100-utils.py >> "$LOGFILE" 2>&1
echo "✅ 2100-utils.py opprettet." | tee -a "$LOGFILE"

# 5. Lag scraper.py
echo "🧠 Genererer 2200-scraper.py v1.1 for FINN.no scraping med pris og beskrivelse..." | tee -a "$LOGFILE"
TEMP_SCRAPER_PY="/tmp/scraper.py.tmp"
cat > "$TEMP_SCRAPER_PY" << 'EOF'
# 2100-scraper.py v1.1
import requests
import re
...
def scrape_finn():
    ...
    log(f"Starter scraping av {url}", log_file='2210-scraper.log')
    try:
        response = requests.get(url, timeout=10)
        response.raise_for_status()
    except requests.RequestException as e:
        log(f"Feil ved henting av {url}: {e}", log_file='2210-scraper.log')
        return
    soup = BeautifulSoup(response.text, 'lxml')
    items = soup.find_all('article')
    conn = get_db_connection()
    if not conn:
        log("Ingen databasetilkobling, avslutter", log_file='2210-scraper.log')
        return
    cur = conn.cursor()
    for item in items:
        ...
        try:
            ...
            log(f"Feil ved henting av beskrivelse for Finn-kode {finn_code}: {e}", log_file='2210-scraper.log')
        try:
            cur.execute(
            ...
            log(f"Feil ved lagring av Finn-kode {finn_code}: {e}", log_file='2210-scraper.log')
            continue
    conn.commit()
    log("Scraping fullført", log_file='2210-scraper.log')
    cur.close()
    conn.close()
...
if __name__ == "__main__":
    scrape_finn()
EOF
if ! sudo mv "$TEMP_SCRAPER_PY" /var/www/finn/2200-scraper.py >> "$LOGFILE" 2>&1; then
    echo "❌ Feil: Kunne ikke opprette 2200-scraper.py." | tee -a "$LOGFILE"
    exit 1
fi
sudo chown www-data:www-data /var/www/finn/2200-scraper.py >> "$LOGFILE" 2>&1
sudo chmod 644 /var/www/finn/2200-scraper.py >> "$LOGFILE" 2>&1
echo "✅ 2200-scraper.py opprettet." | tee -a "$LOGFILE"
